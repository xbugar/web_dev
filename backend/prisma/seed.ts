...
async function createUserWithTags(name: string, mail: string, picture: ProfilePicture, icon: Icon, tag: string, color: string) {
    const user = await addUser(name, "", mail, "", picture.id);
    const tag = await createTag(user.id, tag, color);
